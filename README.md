...
**你不需要先判断该用哪个技能。** 安装完成后，在固件项目里直接描述任务即可；AI 会按请求内容自动加载合适技能、调用配套命令或工具。用 `$技能名` 显式点名是可选的，适合确认路由或强制切换，不是使用前提。

下表帮助你了解 AI 可能用到的入口，以及各自交付什么，**不是一份必须手工选型的清单**：
...
技能提供工程指导，命令提供执行入口，工具处理具体数据。按任务组合，也可独立使用；协议设计并非固件开发、测试或构建的前置步骤。若 AI 选错了入口，直接纠正任务描述或点名所需技能即可。
...
| 使用方式 | 下一步 | 需要什么 |
|---|---|---|
| 在 Codex 使用技能 | 按 [Codex 安装](docs/workbench-usage.md#codex) 复制完整技能目录；之后直接描述任务，由 AI 自动选用（也可 `$技能名` 点名） | Codex；不要求 DSH、Keil 或开发板 |
...
直接描述问题即可，不必先选技能。例如，在已有固件项目中请求 DMA 排障：
...
需要锁定入口时，再在句首加上 `$arm-cortex-expert` 或 `$embedded-test-engineer`。
