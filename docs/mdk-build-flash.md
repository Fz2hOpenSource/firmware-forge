...
slash 插件向包装脚本显式传入 `-WaitTimeoutSec 900`，外层等待为 960 秒，给 UV4 的超时处理与结果收集留出余量。直接调用脚本时可另设 `-WaitTimeoutSec`。该参数约束 UV4 等待，不代表其它烧录后端的内部等待也受它控制；外层中断时报告结果不确定，需要核对进程、日志和目标状态后再决定是否重试。
