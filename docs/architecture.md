...
当前提供 Codex 和 DSH 两种宿主入口；三个技能可独立使用，DSH 工作台提供可选的构建与烧录集成。项目由技能、命令和工具组成，按当前开发、诊断、测试或执行任务选择入口，设计取舍见 [设计理念](design-philosophy.md)。
...
| 技能层 | 固件、测试、协议三个 `SKILL.md` | 分别提供固件实现/排障、测试与回放、交互契约设计能力；见 [技能分工](skills-integration.md)。 |
...
1. 明确当前任务与验收条件，沿用已有需求。单独查询或构建可直接选择工具/命令。
2. 检查项目契约、代码、`.ioc`、链接脚本、日志和厂商资料。冲突需明确，不能用通用技能规则覆盖产品要求或硬件限制。
3. 需要工程判断时选择固件、测试或协议技能；使用对应领域参考，避免把专项设计变成通用前置流程。
