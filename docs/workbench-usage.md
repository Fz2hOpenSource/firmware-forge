...
安装完成后，直接用自然语言描述任务；AI 可按任务选择相应技能、命令或工具。也可以在句首用 `$技能名` 指定入口。下文路径除特别说明外，均相对本仓库根目录。
...
三个技能都允许隐式调用：在固件项目里描述任务后，Codex 可按请求选用已装技能。也可以在句首用 `$技能名` 指定入口。最终是否选中，以实际输出和加载路径为准。
